...
bool Task::fromJsonV0(Task *p_task, const QJsonObject &p_obj, bool p_mergeTasks)
{
    ...
    if (p_obj.contains("inputs")) {
        ...
        for (int i = 0; i < arr.size(); ++i) {
            ...
            if (input.type == "pickString" && inputObj.contains("options")) {
                input.options = getLocaleStringList(inputObj["options"], p_task->m_locale);
            }

            if (input.type == "pickString" && !input.default_.isNull() && !input.options.contains(input.default_)) {
...
const InputDTO &Task::getInput(const QString &p_id) const
{
...
const MessageDTO &Task::getMessage(const QString &p_id) const
{
